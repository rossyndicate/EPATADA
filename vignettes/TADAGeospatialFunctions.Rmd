...
now includes a "geometry" column, which allows for mapping and
additional geospatial capabilities. Enter `?TADA_MakeSpatial` into the
console to review another example of this function in use and additional
information.
...
Users also have the option of loading the ATTAINS geospatial sf objects
into their local environment with `return = TRUE`. The objects returned
are named `ATTAINS_catchments`, `ATTAINS_lines`, `ATTAINS_points`, and
`ATTAINS_polygons`.
...
And, because we set `return = TRUE`, we now have all the raw ATTAINS
features associated with these TADA Water Quality Portal observations
saved in our Global Environment as `ATTAINS_catchments`,
`ATTAINS_lines`, `ATTAINS_points`, and `ATTAINS_polygons` objects.
...
TADA WQP observations. (Essentially, this maps the same data created
from running `TADA_GetATTAINS()`.) For the function to work properly,
the input data frame must be a WQP dataframe developed by
`TADA_DataRetrieval()`.

#### Using `TADA_ViewATTAINS()`

Using either our original `TADA_dataframe` *or* the geospatial version
`TADA_spatial`, we can plot all water quality observations, as well as
the ATTAINS features, using `TADA_ViewATTAINS()` . Enter
`?TADA_ViewATTAINS` into the console to review another example query and
